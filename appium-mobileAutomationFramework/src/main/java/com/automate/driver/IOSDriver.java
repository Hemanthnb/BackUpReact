...
public class IOSDriver implements DriverInterface {

    public static AppiumDriver<MobileElement> createDriver(String deviceName, String udid, int port, String emulator) {
      try {
    ...
    public static AppiumDriver<MobileElement> createIOSDriverForNativeApp(String deviceName, String udid, int port) {
      try {
        ...
        return new io.appium.java_client.ios.IOSDriver<>(new URL(getConfig(ConfigJson.APPIUM_URL)), capability);
      } catch (Exception e) {
        throw new DriverInitializationException("Failed to initialize driver. Please check the desired capabilities", e);
      }
    }
